	...
	return (
		<a href="" className={`relative rounded-lg overflow-hidden ${props.className}`}>
			...
			<div className="w-full pt-16 pb-4 px-4 bg-game-gradient absolute right-0 left-0 bottom-0">
				<strong className="text-sm md:text-base text-white font-bold block">{props.title}</strong>
				<span className="text-sm md:text-base text-zinc-300 text-300 block">{props.adsCount} {props.adsCount == 1 ? 'anúncio' : 'anúncios'}</span>
			</div>
		</a>
	)
}